...
# Validation
if (!(Test-Path -path $SetupFile)) {Write-HYDLog "Could not find SQL Server 2017 Reporting Services Setup files, aborting..." -LogLevel 2;Break}
...
If (! $configset.IsInitialized)
{
    ...
    Import-Module "C:\Program Files (x86)\Microsoft SQL Server\140\Tools\PowerShell\Modules\sqlps\sqlps.psd1" -DisableNameChecking | Out-Null
